...
// Locatoraid CSV field mappings
const LOCATORAID_FIELDS = [
  "name",
  "street1",
  "street2",
  "city",
  ...
  if (shouldWriteHeader) {
    return new Promise((resolve, reject) => {
      ...
      dataArray.forEach((row) => {
        const rowToWrite = mappingFunction ? mappingFunction(row) : row;
        if (!csvStream.writableEnded) csvStream.write(rowToWrite);
      });
    ...
    let fd;
    try {
      const rowsToWrite = mappingFunction 
        ? dataArray.map(row => mappingFunction(row)) 
        : dataArray;
      
      const csvString = await fastcsv.writeToString(rowsToWrite, {
        headers: false,
        writeHeaders: false,
        includeEndRowDelimiter: true,
      });
...
async function processCSV(fromRow = FROM_ROW) {
  ...
  // Generate Locatoraid output path by adding _locatoraid before the extension
  ...
  let actualLocatoraidOutputCsvPath = path.join(
    outputDir,
    `${outputFileName}_locatoraid${outputFileExt}`
  );
  let startRow = 0;
  ...
  if (fromRowArg) {
    ...
    if (!isNaN(fromRowValue) && fromRowValue > 0) {
      ...
      actualLocatoraidOutputCsvPath = path.join(
        outputDir,
        `${outputFileName}-from-row-${fromRowValue}_locatoraid${outputFileExt}`,
      );
  ...
  Locatoraid Output CSV:  ${actualLocatoraidOutputCsvPath}
  ...
  readStream
    ...
    .on("end", async () => {
      ...
      let locatoraidHeadersHaveBeenWritten = false;
      ...
      if (
        fs.existsSync(actualLocatoraidOutputCsvPath) &&
        fs.statSync(actualLocatoraidOutputCsvPath).size > 0
      ) {
        const msg = `Locatoraid output file ${actualLocatoraidOutputCsvPath} exists and is not empty. Assuming headers are present. Will append new data.`;
        console.log(`[INFO] ${msg}`);
        await writeBatchToLogFile(LOG_FILE_PATH, [
          `[${new Date().toISOString()}] INFO: ${msg}`,
        ]);
        locatoraidHeadersHaveBeenWritten = true;
      } else {
        const msg = `Locatoraid output file ${actualLocatoraidOutputCsvPath} does not exist or is empty. Headers will be written with the first batch.`;
        console.log(`[INFO] ${msg}`);
        await writeBatchToLogFile(LOG_FILE_PATH, [
          `[${new Date().toISOString()}] INFO: ${msg}`,
        ]);
        ...
        if (
          ...
          if (processedRowsBatch.length > 0) {
            try {
              ...
              // Write to Locatoraid CSV
              const shouldWriteLocatoraidHeadersForThisBatch = !locatoraidHeadersHaveBeenWritten;
                
              // Mapping function to convert standard rows to Locatoraid format
              const mapToLocatoraid = (row) => {
                return {
                  name: row.nombre || '',
                  street1: row.domicilio || '',
                  street2: '',  // field mapping doesn't specify a direct source for street2
                  city: row.localidad || '',
                  state: row.provincia || '',
                  zip: row.c_postal || '',
                  country: row.pais || '',
                  phone: '',    // field mapping doesn't specify a source for phone
                  website: '',  // field mapping doesn't specify a source for website
              ...
              console.log(
                `[INFO] Writing batch of ${processedRowsBatch.length} rows to Locatoraid CSV. Input rows up to ${currentRowInInputFile}.`,
              );
                
              await writeBatchToCsv(
                actualLocatoraidOutputCsvPath,
                processedRowsBatch,
                LOCATORAID_FIELDS,
                shouldWriteLocatoraidHeadersForThisBatch,
                mapToLocatoraid
              );
                
              if (shouldWriteLocatoraidHeadersForThisBatch) {
                locatoraidHeadersHaveBeenWritten = true;
              }
      ...
      const loopEndMsg = `Processing loop finished. Outputs saved to "${actualOutputCsvPath}" and "${actualLocatoraidOutputCsvPath}". Total rows from input: ${rows.length}. Records geocoded in this run: ${geocodedCountInThisRun}.`;
